#!/usr/bin/env python3
"""
run_deepseek.py — SSEF Phase 1, DeepSeek generation run
=======================================================

Project : A Quality Evaluation Framework for Sinhala Story Generation (SSEF)
Version : 1.0
Created : 2026-08-19
Requires: prompts_v3.py in the same directory (gate must be (720,880),
          MAX_ATTEMPTS 4)
Install : pip install requests

WHY DEEPSEEK (DEC-031)
----------------------
Replaces Kimi, rejected under DEC-030: kimi-k2.6 consumed its entire token
budget on reasoning regardless of size (9,000 -> 9,115 reasoning tokens;
30,000 -> 29,999) and returned content on only 3 of 10 production topics.

DeepSeek preserves the DEC-025 rationale — a non-Western training corpus,
giving RQ4 a genuine contrast rather than three variants of the same regime.

Measured 2026-08-19, deepseek/deepseek-v4-pro, N01_CoastTrain, variant A2:
    reasoning OFF (default) : 4,511 completion tokens, 0 reasoning, 102 s, $0.011
    reasoning ON            : 7,021 completion, 4,293 reasoning, 137 s, $0.028
Both returned real Sinhala with finish_reason "stop".

⚠️ REASONING IS NOT ON BY DEFAULT FOR THIS MODEL.
It must be requested explicitly. Gemini reasons by default; DeepSeek does not.
Both are now set EXPLICITLY rather than left at provider defaults — a
deliberate deviation from the "provider defaults untouched" stance elsewhere
in the parameter notes. STATE THIS IN THE METHODOLOGY.

WHY REASONING MUST BE ON (DEC-027, confirmed on evidence)
---------------------------------------------------------
...
No overlap between the two sets. Reasoning mode materially changes output
length, so it cannot differ across models without confounding RQ4.

SETUP (Windows PowerShell)
--------------------------
    $env:OPENROUTER_API_KEY="sk-or-v1-..."
    python run_deepseek.py generate 2>&1 | Tee-Object -FilePath run_log_ds.txt

SETUP (macOS / Linux)
---------------------
    export OPENROUTER_API_KEY="sk-or-v1-..."
    python run_deepseek.py generate 2>&1 | tee run_log_ds.txt

NEVER hardcode the key. Never commit it. Never paste it into a chat.

STAGES
------
    python run_deepseek.py probe      # ONE story — read it before anything else
    python run_deepseek.py refusal    # 10 truncated calls, pennies
    python run_deepseek.py generate   # full run, ~25 min, well under $1
"""
...
# Excluded: ~deepseek-v4-flash-latest (moving alias — destroys
# reproducibility), r1-distill-llama-70b (distill, 8K context),
# v3.2-exp (experimental), r1/r1-0528 (older generation).
MODEL_ID = "deepseek/deepseek-v4-pro"
MODEL_ID_READ_ON = "2026-08-19"
# Backup with better provenance if v4-pro drifts: deepseek/deepseek-v4-pro-0813
...
OUT = Path("outputs")
STORY_DIR = OUT / "stories" / "deepseek"   # one folder per model; identical
                                           # filenames across folders make the
                                           # crossed design visible on disk
RAW_DIR = OUT / "raw_deepseek"
...
def probe(topic_id="N12_ITNightShift"):
    ...
    (OUT / "probe" / f"deepseek_{topic_id}.txt").write_text(clean, encoding="utf-8")
    print("\n" + "=" * 60 + "\n")
    print(clean)
    print("\n" + "=" * 60)
...
def refusal_screen():
    ...
    for tid in TOPICS:
        ...
        _write_csv(OUT / f"refusal_deepseek_{RUN_ID}.csv", rows)
        time.sleep(SLEEP_BETWEEN)

    print(f"Saved outputs/refusal_deepseek_{RUN_ID}.csv — read every row.")
...
def generate():
    ...
    SAVES (written incrementally — a crash loses at most one attempt)
      ...
      outputs/manifest_deepseek_{run}.jsonl     one line per accepted story
      outputs/attempts_deepseek_{run}.csv       EVERY attempt, rejections too
      outputs/raw_deepseek/{topic}_a{n}.txt     raw response, pre-strip
    ...
    for idx, tid in enumerate(TOPICS, 1):
        ...
        for n in range(1, MAX_ATTEMPTS + 1):
            ...
            if raw is None:
                ...
                _write_csv(OUT / f"attempts_deepseek_{RUN_ID}.csv", attempts)
        ...
        _write_jsonl(OUT / f"manifest_deepseek_{RUN_ID}.jsonl", manifest)
    ...
    print(f"  Ten DeepSeek stories are ONE THIRD of a corpus, not a result.")
